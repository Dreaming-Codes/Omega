...
namespace Code {
    ...
    MenuController::MenuController(Responder *parentResponder, App *pythonDelegate, ScriptStore *scriptStore,
    ...
    void MenuController::duplicateScript(Script script) {
        ...
        Ion::Storage::sharedStorage()->createRecordWithExtension(buffer, Code::ScriptStore::k_scriptExtension,
                                                                 script.value().buffer, script.value().size);
    ...
    void MenuController::tableViewDidChangeSelection(SelectableTableView *t, int previousSelectedCellX,
    ...
    bool MenuController::textFieldDidFinishEditing(TextField *textField, const char *text, Ion::Events::Event event) {
        ...
        static constexpr int bufferSize =
                Script::k_defaultScriptNameMaxSize + 1 + ScriptStore::k_scriptExtensionLength; //"script99" + "." + "py"
        ...
        if (strlen(text) > 1 + strlen(ScriptStore::k_scriptExtension)) {
            ...
            strlcpy(numberedDefaultName + defaultNameLength, ScriptStore::k_scriptExtension,
        ...
        Script::ErrorStatus error = Script::nameCompliant(newName) ? m_scriptStore->scriptAtIndex(
                m_selectableTableView.selectedRow()).setName(newName) : Script::ErrorStatus::NonCompliantName;
        if (error == Script::ErrorStatus::None) {
            ...
            Container::activeApp()->displayWarning(I18n::Message::AllowedCharactersaz09,
    ...
    bool
    MenuController::privateTextFieldDidAbortEditing(TextField *textField, bool menuControllerStaysInResponderChain) {
        ...
        if (strlen(scriptName) <= 1 + strlen(ScriptStore::k_scriptExtension)) {
            ...
            Script::ErrorStatus error = script.setBaseNameWithExtension(numberedDefaultName,
        ...
        if (menuControllerStaysInResponderChain) {
            m_selectableTableView.selectCellAtLocation(m_selectableTableView.selectedColumn(),
