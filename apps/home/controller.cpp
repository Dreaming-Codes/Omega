...
namespace Home {
    ...
    Controller::ContentView::ContentView(Controller *controller, SelectableTableViewDataSource *selectionDataSource) :
        ...
        static_cast<ScrollView::BarDecorator *>(m_selectableTableView.decorator())->verticalBar()->setMargin(
                k_indicatorMargin);
    }
        ...
        if (numberOfIcons % numberOfColumns) {
            ...
            for (int i = 0; i < numberOfColumns; i++) {
                if (i >= numberOfIcons % numberOfColumns) {
                    markRectAsDirty(
                            KDRect(dataSource->cellWidth() * i, dataSource->cellHeight(), dataSource->cellWidth(),
                                   dataSource->cellHeight()));
                }
            }
        }
    }
    ...
    void Controller::ContentView::layoutSubviews(bool force) {
        ...
        m_backgroundView.setFrame(
                KDRect(0, Metric::TitleBarHeight, Ion::Display::Width, Ion::Display::Height - Metric::TitleBarHeight),
                force);
    ...
    bool Controller::handleEvent(Ion::Events::Event event) {
        ...
        if (event == Ion::Events::Left && selectionDataSource()->selectedRow() > 0) {
            return m_view.selectableTableView()->selectCellAtLocation(numberOfColumns() - 1,
                                                                      selectionDataSource()->selectedRow() - 1);
        }
    ...
    void Controller::willDisplayCellAtLocation(HighlightCell *cell, int i, int j) {
        ...
        if (appIndex >= container->numberOfApps()) {
            ...
            ::App::Descriptor *descriptor = container->appSnapshotAtIndex(
                    PermutedAppSnapshotIndex(appIndex))->descriptor();
